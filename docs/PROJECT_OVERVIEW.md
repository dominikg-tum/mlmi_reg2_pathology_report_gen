...
| ⬜ Open   | **Re-tile + re-encode @ `cache_20x_v2`** — leave legacy `dominik/cache` alone; write HSV + stratified 20× CONCH into `dominik/cache_20x_v2` keyed by `TUM_Uterus_XXXX.svs` (`wsi_name_map.csv`)                                                                                                              | DOMI                   |
...
| ❌ Dropped (v1) | **Graph escape / `novel_finding_capture`** — removed from `execution_graph.jsonl`; `none_of_above` routes to `synthesis_interpretation`. Taxonomy-escape overlays deferred ([§2j](#2j-graph-coverage-escape-hatches-optional))                                                                              | —                      |
