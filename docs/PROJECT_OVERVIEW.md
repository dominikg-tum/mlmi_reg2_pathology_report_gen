...
| ⬜ Open   | **Mag simplification (target)** — offline **20× only** CONCH pool; `thumbnail_only` global nodes; retriever ignores `zoom_level` for pool routing ([§2h](#2h-simplified-magnification--offline-cost))                                                                                                        | DOMI                   |
| ⬜ Open   | `vision.yaml` — `encode_levels: [20x]`; `retrieval.fixed_pool: 20x`; disable `adjacent_scale`                                                                                                                                                                                                                | DOMI                   |
...
| ⬜ Open   | **PathAgent prompt pack** — `agent/prompts.py`: Perceptor + Step A/B/C templates ([§2g](#2g-pathagent-style-bounded-react-per-graph-node))                                                                                                                                                                   | DOMI                   |
| ⬜ Open   | **Structured node answers** — `backends.py`: preamble + JSON `{answer_key, rationale, confidence}` via `--node-react` or `--structured-answer`                                                                                                                                                               | DOMI                   |
| ⬜ Open   | **Per-node ReAct loop** — `node_react.py` + `controller.py` flags; B/C dispatch; `node_traces[]` in `cot_chain`; wire `Node.retrieval_text_with_context(prior_steps, sub_query=I_t)` on re-retrieve ([§2g](#2g-pathagent-style-bounded-react-per-graph-node), `graph/schema.py`)                              | DOMI                   |
| ⬜ Open   | **Runtime 40× zoom tool** — `wsi_io.py` + ReAct zoom branch; no offline `patch_embeddings_40x.pt`                                                                                                                                                                                                            | DOMI                   |
| ⬜ Open   | **Phase 2 report prompts** — `report_writer.py`: PathAgent observable-findings rules (no glossaries)                                                                                                                                                                                                         | XUN                    |
| ⬜ Open   | **Retrieval coord logging** — patch coords in `cot_chain.json` / `node_traces[]` per patch node                                                                                                                                                                                                              | DOMI                   |
...
| Per-node ReAct                    | **Planned** — `agent/prompts.py` + `node_react.py` + `--node-react` ([§2g](#2g-pathagent-style-bounded-react-per-graph-node)) | Steps A/B/C; `--structured-answer` for prompt-only baseline                                                                                                                                                                   |
...
| Per-node ReAct                          | **Planned** — `prompts.py` / `node_react.py` / `backends.py` ([§2g](#2g-pathagent-style-bounded-react-per-graph-node))                                                                       |                                                                               |
...
| **Magnification should match question granularity**          | ⚠️ **Simplified:** fixed 20× retrieve + thumbnail global; `zoom_level` = doc hint; ReAct 40× runtime zoom ([§2h](#2h-simplified-magnification--offline-cost)) |
| **Cross-magnification context (CMT)**                        | ⚠️ **Thumbnail + 20× patches** replaces offline parent pools when on single-pool mode                                                                         |
...
| ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| **Per-node evidence sufficiency** (reflect before committing)     | ✅ Planned: bounded ReAct inside each graph node — [§2g](#2g-pathagent-style-bounded-react-per-graph-node) |
| **Sub-query retrieval** `I_t` on unvisited / excluded patches     | ✅ `exclude` in retriever + `node.retrieval_text` + `I_t`                                                  |
| **Adaptive zoom** within a region (Action 2)                      | ✅ Child zoom pool or on-the-fly 40× crop around best coord                                                |
