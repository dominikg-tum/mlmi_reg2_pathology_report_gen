...
def test_extract_lesion_patches_writes_top_patch_at_selected_level(monkeypatch, tmp_path: Path):
    svs = tmp_path / "CASE.svs"
    svs.write_text("not a real slide")

    def fake_iter(*_args, **kwargs):
        assert kwargs["objective"] == "1.25x"
    ...
    manifest = lesion_export.extract_lesion_patches(
        svs_path=svs,
    ...
    assert manifest["level"] == "1.25x"
    assert (tmp_path / "cache" / "CASE.svs" / "lesion_patches_1p25x.json").exists()
