"""Export likely lesion patches using PathAgent-style PLIP retrieval."""
...
DEFAULT_LEVEL = "5x"
VALID_LEVELS = ("1.25x", "2.5x", "5x", "10x")
...
def extract_lesion_patches(
    *,
    ...
    """Tile one WSI, score tissue patches, and save top lesion candidates."""
    slide_id = slide_id_from_path(svs_path)
    out_dir = slide_cache_dir(cache_root, slide_id)
    safe_level = _safe_level(level)
    patch_dir = out_dir / f"lesion_patches_{safe_level}"
    ...
    if not patches:
        raise RuntimeError(f"No tissue patches found at {level} for {svs_path}")
    ...
    manifest_path = out_dir / f"lesion_patches_{safe_level}.json"
    manifest_path.write_text(json.dumps(manifest, indent=2) + "\n")
    return manifest


def extract_lesion_patches_5x(**kwargs) -> dict:
    """Backward-compatible wrapper for older callers/tests."""
    return extract_lesion_patches(level="5x", **kwargs)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract likely lesion patches using PathAgent-style PLIP retrieval."
    )
    ...
    parser.add_argument("--level", choices=list(VALID_LEVELS), default=DEFAULT_LEVEL)
    ...
    for svs_path in svs_files:
        try:
            manifest = extract_lesion_patches(
                svs_path=svs_path,
